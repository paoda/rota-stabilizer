...
/// Scale a length authored at 100% (96 DPI) into the current display's units.
///
/// NB: Prefer deriving from calcTextSize() for anything that has to bound text
pub fn scaled(value: f32) f32 {
    const style = getStyle().inner;

    // deliberately not ImGui_GetFontSize() / FontSizeBase: the former is rounded to an integer
    return value * style.FontScaleMain * style.FontScaleDpi;
}
...
pub const backend = struct {
    ...
    pub fn init(window: *c.SDL_Window, gl_ctx: c.SDL_GLContext) void {
